    ...
    public void register() {
        ServerPlayConnectionEvents.JOIN.register((handler, sender, server) -> {
            ...
            if (!EasyBotFabric.getBridgeClient().isReady()) {
                EasyBotFabric.LOGGER.warn("玩家取消玩家上线同步,因为服务器未连接主程序");
                return;
            }
            var playerInfo = PlayerUtils.getPlayerInfo(player); // 注意,最好不要在别的线程获取,你永远不知道下一个tick数据是否可用
            EasyBotNetworkingThreadPool.getInstance().addTask(() -> EasyBotFabric.getBridgeClient().syncEnterExit(playerInfo, true), "消息同步-进服");
        });
        ...
        ServerPlayConnectionEvents.DISCONNECT.register((handler, server) -> {
            ...
            if (!EasyBotFabric.getBridgeClient().isReady()) {
                EasyBotFabric.LOGGER.warn("玩家取消玩家下线同步,因为服务器未连接主程序");
                return;
            }
            var playerInfo = PlayerUtils.getPlayerInfo(player); // 注意,最好不要在别的线程获取,你永远不知道下一个tick数据是否可用
            EasyBotNetworkingThreadPool.getInstance().addTask(() -> EasyBotFabric.getBridgeClient().syncEnterExit(playerInfo, false), "消息同步-退服");
        });
    }
}
